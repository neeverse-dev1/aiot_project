...
def simulate_distance():
    #HC-SR04 센서 데이터를 시뮬레이션하는 함수
    #특정 간격마다 이상치(급격한 거리 변화) 발생
    global sim_current_distance
    global sim_anomaly_trigger_counter
...
# 1. '정상' 데이터 수집을 위한 설정
#    모델 훈련을 위해 수집할 정상 데이터 포인트의 수
#    이 데이터가 모델이 '정상'이라고 판단할 기준
NUM_TRAINING_SAMPLES = 100 
# 데이터를 수집할 때 각 측정 사이의 시간 간격 (초)
TRAINING_COLLECTION_INTERVAL = 0.5 

# 2. Isolation Forest 모델 설정
#    contamination: 데이터셋에서 이상치(anomaly)의 비율을 추정하는 값 (0과 0.5 사이).
#                   이 값을 조절하여 이상 감지의 민감도를 조절
#    random_state: 재현 가능한 결과를 위해 설정
...
try:
    ...
    # 수집된 정상 데이터를 사용하여 Isolation Forest 모델을 훈련
    print("\n-------------------------------------------------")
    ...
    # interval은 밀리초 단위 (예: 500ms마다 업데이트)
    ...
    while True:
        ...
        if current_distance is not None:
            ...
            # 모델을 사용하여 현재 샘플이 이상치인지 예측
            # predict() 메서드는 1 (정상) 또는 -1 (이상치)를 반환
